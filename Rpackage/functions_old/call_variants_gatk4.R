...
unfilteredVCF <- file.path(outvariants,caller,region_call,paste0(sampleName,"_",type_call,"_snvs_indels_unfiltered.vcf")) # only needed for Mutect2 at this stage
...
if (call) {
	...
	if ( caller == "mutect" ) {
		...
		mutect_call <- paste0('gatk Mutect2 ',
			" -R ", genome, 
			" -I ", bamfile, 
			" -tumor ", sampleName)
		...
		if (!is.null(opt$panel_of_normals)){
			...
			mutect_call <- paste0(mutect_call," --panel-of-normals ", opt$panel_of_normals)
		...
		if (!is.null(opt$regions)){
			...
			mutect_call <- paste0(mutect_call," -L ", opt$regions, " -O ",unfilteredVCF)

		} else { mutect_call <- paste0(mutect_call," -O ",unfilteredVCF) }

		print(mutect_call)
		system(mutect_call)
		print(paste0("Variant called with Mutect2 for ",basename(unfilteredVCF),"."))

		# Filter mutect2calls
		mutect2_filters <- paste0('gatk FilterMutectCalls ',
			" -V ", unfilteredVCF, 
			" -O ", VCF)
		print(mutect2_filters)
		system(mutect2_filters)
		print(paste0("Variants filtered from Mutect2 for ",basename(VCF),"."))
	...
	if ( caller == "vardict" ) {
		...
		# Build samtools call
		vardict_call <- paste0("vardict -f 0.01 -c 1 -S 2 -E 3 -g 4 -r 2 -t -th 10 -v ",
			"-G ",genome)
		...
		vardict_call <- paste0(vardict_call," ",opt$regions, " | ",
			...
			file.path(opt$VarDict_dir,"var2vcf_valid.pl")," -N ",sampleName, " -E -f 0.01 > " , VCF)
