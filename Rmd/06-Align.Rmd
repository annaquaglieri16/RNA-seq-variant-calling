...
In order, `post_align_qc2.sh` arguments are:
  ...
  4. `SampleName of the run`. If a sample was sequenced across different lanes you need to set lane-specific read groups to each separate bamfile (e.g. SampleName_L1, SampleName_L2). This sample name will be used for the fields `RGLB` and `RGSM` in the `AddOrReplaceReadGroups` groups below. See Section \@(sec:merge-bamfiles) for merging bamfiles.
